...
from fbpic.utils.mpi import MPI, mpi_installed, gpudirect_enabled, comm
...
class Writer:
    """ Class to direct print calls to terminal or/and file """
    
    def __init__(self, filename=f'{comm.rank}.out' ):

        # flags for printing
        if os.environ.get('FBPIC_WRITE_TO_STDOUT', default='1') == '1':
            write_to_stdout= True
        else:
            write_to_stdout = False
            
        if os.environ.get('FBPIC_WRITE_TO_FILE', default='0') == '1':
            write_to_file = True
        else:
            write_to_file = False
        
        if os.environ.get('FBPIC_PREPEND_RANK_TO_STDOUT', default='0') == '1':
            prepend_rank = True
        else:
            prepend_rank = False
        ...
        self.prepend_rank = prepend_rank
    ...
    def file(self, mode='r'):
        self.f = open(self.filename, mode=mode, encoding=sys.stdout.encoding)
        return self.f
    ...
    def __call__(self, *args, end='\n', sep=' ', file=None, flush=False ):
        """
        Mimic the print statement in constructing the string, silently ignore 
        the `file` kwarg.
        
        """
        # construct the string
        string = sep.join([str(a) for a in args]) + end
        if self.prepend_rank:
            string = f'({comm.rank}) ' + string
...
class ProgressBar(object):
    ...
    def print_progress( self ):
        ...
        if i == 0:
            ...
            print('\r|' + nbars*self.bar_char, end='' )
            print((self.Nbars-nbars)*' ', end='')
            print('| %d/%d' %(i+1,self.N), end='')
            ...
            if self.eta is None:
                ...
                print(', %d:%02d:%02d left' % (h, m, s), end='')
            # Time taken by the last step
            #sys.stdout.write(', %d ms/step' %(self.time_per_step*1.e3))
            #sys.stdout.flush()
            print(', %d ms/step' %(self.time_per_step*1.e3), end='', flush=True)
        # Clear line
